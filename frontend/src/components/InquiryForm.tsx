...
const InquiryForm = ({
  ...
  const handleSubmit = async (
    ...
    const apiUrl =
      (import.meta.env as any)
        .VITE_INQUIRY_API ||
      "http://localhost:4000/api/inquiry";
    ...
    return (
      ...
      >
        ...
        <p className="font-body text-muted-foreground text-sm leading-relaxed">
          ...
          Your travel request is now
          with our team, and we’ll
          connect with tailored
  ...
  const inputClasses =
    "w-full px-5 py-3.5 bg-[#111827] rounded-xl font-body text-sm text-primary-foreground placeholder:text-primary-foreground/50 border border-primary-foreground/10 focus:border-gold/50 focus:ring-1 focus:ring-gold/20 outline-none transition-all backdrop-blur-md";

  return (
    <form
      onSubmit={handleSubmit}
      className="space-y-5 premium-card p-6 lg:p-8"
      id="inquiry-form"
    >
      {/* NAME + PHONE */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        ...
        >
          ...
          <option value="Himachal Adventure">
            Himachal Adventure
          </option>
          ...
          <option value="Dubai Family Trip">
            Dubai Family Trip
          </option>
      ...
      <button
        type="submit"
        disabled={sending}
        className="w-full py-3.5 bg-gradient-to-r from-gold-dark via-gold to-gold-dark text-secondary-foreground font-body font-semibold text-sm rounded-xl transition-all duration-300 tracking-[0.15em] uppercase hover:shadow-lg hover:shadow-gold/15 disabled:opacity-60 disabled:cursor-not-allowed"
      >
        {sending
          ? "Sending..."
          : "Send Inquiry"}
